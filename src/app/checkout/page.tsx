...
function CheckoutContent() {
  ...
  useEffect(() => {
    ...
    if (targetOption) {
      const itemPayload = {
        ...
        requirements: targetOption.requirements,
      };
  ...
  const renderServiceOption = (option: CheckoutOption) => {
    ...
    return (
      <div
        key={option.id}
        className={`flex items-start gap-3 py-4 border-b border-gray-100 last:border-0 transition-colors ${quantity > 0 ? 'bg-red-50/20 -mx-4 px-4 sm:mx-0 sm:px-0' : 'bg-transparent'
          }`}
      >
        {/* Content Section */}
        ...
        <div className="flex flex-col items-center gap-1 shrink-0 ml-2 mt-1">
          {quantity > 0 ? (
            ...
            <button
              ...
              className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all shadow-sm ${checkoutType === 'direct' && !provider
