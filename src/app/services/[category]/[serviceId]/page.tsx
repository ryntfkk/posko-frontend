            ...
            <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
                ...
                <section>
                    <div className="flex items-center justify-between mb-3 px-1">
                        <h3 className="text-sm font-bold text-gray-900">Mitra Tersedia ({recommendProviders.length})</h3>
                        <span className="text-[10px] text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">Sekitar Anda</span>
                    </div>

                    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                        {recommendProviders.length === 0 ? (
                            <div className="col-span-full p-6 text-center bg-white rounded-xl border border-dashed border-gray-200">
                                <p className="text-xs text-gray-500">Belum ada mitra spesifik untuk layanan ini di lokasi Anda.</p>
                            </div>
                        ) : (
                            recommendProviders.map(prov => (
                                <button
                                    key={prov._id}
                                    onClick={() => {
                                        // Different Order Flow: Direct Order
                                        resetAndAddItem({
                                            // id: service._id, // Removed to fix lint error (Omit<CartItem, "id">)
                                            serviceId: service._id,
                                            serviceName: service.name,
                                            category: categoryParam || 'general',
                                            pricePerUnit: service.displayPrice || service.basePrice,
                                            ...
                                            orderType: 'direct',
                                            providerId: prov._id,
                                            providerName: prov.userId?.fullName,
                                            duration: service.estimatedDuration || 60,
                                            requirements: service.requirements
                                        });
                                        router.push(`/order/summary?type=direct&providerId=${prov._id}&category=${categoryParam}`);
                                    }}
                                    className="bg-white rounded-xl border border-gray-100 shadow-[0_2px_8px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col w-full group hover:shadow-md hover:border-red-100 transition-all duration-300 relative text-left"
                                >
                                    ...
                                    <div className="p-2.5 flex flex-col gap-1 w-full">
                                        ...
                                        <div className="flex justify-between items-start gap-2">
                                            <h3 className="text-sm font-bold text-gray-900 leading-tight line-clamp-1 group-hover:text-red-600 transition-colors">
                                        ...
                                        <p className="text-[10px] text-gray-500 line-clamp-1 flex items-center gap-1">
                                            ...
                                            {calculateDistance(currentLocation?.lat || 0, currentLocation?.lng || 0, prov.userId?.location?.coordinates[1] || 0, prov.userId?.location?.coordinates[0] || 0)} km
                                        </p>

                                        {/* STARTING PRICE */}
                                        <div className="mt-2 pt-2 border-t border-gray-50 flex items-center justify-between">
                                            <div className="flex flex-col">
                                                <span className="text-[9px] text-gray-400 font-medium">Layanan Ini</span>
                                                <span className="text-xs font-bold text-red-600">
                                                    {new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(service.displayPrice || service.basePrice)}
                                                </span>
                                            </div>
                                            <div className="w-6 h-6 rounded-full bg-red-50 text-red-600 flex items-center justify-center group-hover:bg-red-600 group-hover:text-white transition-all">
